...
//Pins
int rxSS = 9;//Connect to BT TX
int txSS = 10; //Connect to BT RX
int meatProbe = 0; //Analog input
...
{  
  ...
  {
    lcd.clear();
    lcd.print("Pit Temp: " + thermister_temp(pitProbe));
    lcd.setCursor(0, 1);
    lcd.print("Meat Temp: " + thermister_temp(meatProbe));
    publishMeatTemp();
    publishPitTemp();
    count = 0;
  }
...
void publishPitTemp()
{
  mySerial.write("pit=" + thermister_temp(analogRead(pitProbe)));
}

void publishMeatTemp()
{
    mySerial.write("meat=" + thermister_temp(analogRead(meatProbe)));
}
